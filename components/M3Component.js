    ...
    template: `
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div className="flex justify-between items-center mb-1 border-b pb-3">
                    <h2 class="text-lg font-bold text-gray-700 flex items-center gap-2">
                        <span className="bg-violet-100 text-violet-800 text-xs px-2 py-1 rounded-full font-semibold">M3 Modeli</span>
                        KPI So'rovnomasi — Noravshan Regressiya (TFN)
                    </h2>
                </div>

                <p className="text-[11px] text-gray-500 mt-2 mb-4 bg-violet-50 border border-violet-100 rounded-lg p-2.5 leading-relaxed">
                ...
                <div className="space-y-3">
                    <div v-for="(q, idx) in questions" :key="q.id"
                        ...
                        <div className="flex-1 min-w-0">
                            <span className="font-bold text-violet-700 font-mono text-xs mr-1">{{ q.id }}.</span>
                            <span className="text-xs text-gray-700 leading-snug">{{ q.label }}</span>
                        </div>
        ...
        <details className="bg-white rounded-xl shadow-sm border border-gray-100 group overflow-hidden transition-all duration-300">
            <summary className="list-none p-6 flex items-center justify-between cursor-pointer hover:bg-gray-50/50 transition-colors select-none">
                <h3 className="text-sm font-bold text-gray-700 flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-violet-500"></span>
                    TFN Qiymatlari Jadvali — Uchburchak ko'rinishidagi tegishlilik funksiyasi
                </h3>
                <svg 
                    className="w-5 h-5 text-gray-400 transform group-open:rotate-180 transition-transform duration-200 flex-shrink-0" 
                    width="20" 
                    height="20" 
                    fill="none" 
                    viewBox="0 0 24 24" 
                    ...
                    <table className="w-full text-xs text-center border-collapse">
                        <thead>
                            <tr className="bg-violet-50">
                                <th className="border border-violet-200 px-2 py-2 text-violet-800 font-bold">Shaxs (xᵢ)</th>
                                <th className="border border-violet-200 px-2 py-2 text-violet-800 font-bold">Baho</th>
                                <th className="border border-violet-200 px-2 py-2 text-violet-800 font-bold">Baho nomi</th>
                                <th className="border border-violet-200 px-2 py-2 text-violet-800 font-bold">α (chap)</th>
                                <th className="border border-violet-200 px-2 py-2 text-violet-800 font-bold">β (o'rta)</th>
                                <th className="border border-violet-200 px-2 py-2 text-violet-800 font-bold">μ (o'ng)</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="row in tfnRows" :key="row.key" className="hover:bg-gray-50 transition-colors">
                                <td className="border border-gray-200 px-2 py-1.5 font-bold font-mono text-violet-700">{{ row.key }}</td>
                                <td className="border border-gray-200 px-2 py-1.5">
                                    <span className="font-bold font-mono px-1.5 py-0.5 rounded text-white text-[11px]"
                                          :style="{ backgroundColor: row.color }">{{ row.rating }}</span>
                                </td>
                                <td className="border border-gray-200 px-2 py-1.5 font-semibold" :style="{ color: row.color }">{{ row.label }}</td>
                                <td className="border border-gray-200 px-2 py-1.5 font-mono">{{ row.alpha.toFixed(2) }}</td>
                                <td className="border border-gray-200 px-2 py-1.5 font-mono">{{ row.beta.toFixed(2) }}</td>
                                <td className="border border-gray-200 px-2 py-1.5 font-mono font-bold text-violet-700">{{ row.mu.toFixed(2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <details className="bg-white rounded-xl shadow-sm border border-gray-100 group overflow-hidden transition-all duration-300">
            <summary className="list-none p-6 flex items-center justify-between cursor-pointer hover:bg-gray-50/50 transition-colors select-none">
                <h3 className="text-sm font-bold text-gray-700 flex items-center gap-2">
                    ...
                    Qurilgan Noravshan Modelning Ifodalanishi
                </h3>
                <svg 
                    className="w-5 h-5 text-gray-400 transform group-open:rotate-180 transition-transform duration-200 flex-shrink-0" 
                    width="20" 
            ...
            <div className="px-6 pb-6 space-y-4 border-t border-gray-50 pt-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p className="text-[11px] text-blue-600 font-semibold mb-1.5 uppercase tracking-wide">Markaziy qiymat (μ̃_y):</p>
                    <div className="font-mono text-[11px] text-blue-900 leading-relaxed break-all">
                        μ̃_y = 72.98 + 7.99·x₁ + 9.77·x₂ + 0.76·x₃ + 0.75·x₄ + 0.77·x₅ + 0.80·x₆ + 0.74·x₇ + 0.79·x₈ + 0.72·x₉
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-4">
                        <div className="text-center">
                            <span className="text-[10px] text-blue-500 block">Asl formula natijasi</span>
                            <span className="text-lg font-bold font-mono text-blue-500">{{ fuzzyResult.mu_y_raw.toFixed(4) }}</span>
                        </div>
                        <div className="text-[10px] text-gray-400">→ normalizatsiya →</div>
                        <div className="text-center">
                            <span className="text-[10px] text-blue-700 block font-semibold">Normallashgan [0, 1]</span>
                            <span className="text-2xl font-black font-mono text-blue-700">{{ fuzzyResult.mu_y.toFixed(4) }}</span>
                        </div>
                        <div className="ml-auto text-[10px] text-blue-400 font-mono">
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                    <p className="text-[11px] text-orange-600 font-semibold mb-1.5 uppercase tracking-wide">Chap koeffitsient (ã_y):</p>
                    <div className="font-mono text-[11px] text-orange-900 leading-relaxed break-all">
                        ã_y = 65.68 − 0.45·x₁ + 0.30·x₂ + 0.32·x₃ + 0.28·x₄ + 0.39·x₅ + 0.39·x₆ + 0.48·x₇ + 0.65·x₈ − 0.73·x₉
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-4">
                        <div className="text-center">
                            <span className="text-[10px] text-orange-500 block">Asl formula natijasi</span>
                            <span className="text-lg font-bold font-mono text-orange-500">{{ fuzzyResult.alpha_y_raw.toFixed(4) }}</span>
                        </div>
                        <div className="text-[10px] text-gray-400">→ normalizatsiya →</div>
                        <div className="text-center">
                            <span className="text-[10px] text-orange-700 block font-semibold">Normallashgan [0, 1]</span>
                            <span className="text-2xl font-black font-mono text-orange-700">{{ fuzzyResult.alpha_y.toFixed(4) }}</span>
                        </div>
                        <div className="ml-auto text-[10px] text-orange-400 font-mono">
                ...
                <div className="bg-cyan-50 border border-cyan-200 rounded-lg p-4">
                    <p className="text-[11px] text-cyan-600 font-semibold mb-1.5 uppercase tracking-wide">O'ng koeffitsient (β̃_y):</p>
                    <div className="font-mono text-[11px] text-cyan-900 leading-relaxed break-all">
                        β̃_y = 58.25 − 0.52·x₁ + 0.29·x₂ + 0.34·x₃ − 0.40·x₄ + 0.39·x₅ + 0.27·x₆ + 0.08·x₇ + 0.67·x₈ − 0.05·x₉
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-4">
                        <div className="text-center">
                            <span className="text-[10px] text-cyan-500 block">Asl formula natijasi</span>
                            <span className="text-lg font-bold font-mono text-cyan-500">{{ fuzzyResult.beta_y_raw.toFixed(4) }}</span>
                        </div>
                        <div className="text-[10px] text-gray-400">→ normalizatsiya →</div>
                        <div className="text-center">
                            <span className="text-[10px] text-cyan-700 block font-semibold">Normallashgan [0, 1]</span>
                            <span className="text-2xl font-black font-mono text-cyan-700">{{ fuzzyResult.beta_y.toFixed(4) }}</span>
                        </div>
                        <div className="ml-auto text-[10px] text-cyan-400 font-mono">
                ...
                <div className="bg-violet-50 border border-violet-200 rounded-lg p-4">
                    <p className="text-[11px] text-violet-600 font-semibold mb-2 uppercase tracking-wide">
                        Yakuniy TFN natija — Ỹ = (ã_y ; μ̃_y ; β̃_y) &nbsp;∈&nbsp; [0, 1]:
                    </p>
                    <div className="flex flex-wrap gap-4 justify-around text-center">
                        <div>
                            <span className="text-[10px] text-violet-500 block">ã_y (chap)</span>
                            <span className="text-xl font-black font-mono text-orange-600">{{ fuzzyResult.alpha_y.toFixed(4) }}</span>
                        </div>
                        <div className="border-l border-r border-violet-200 px-6">
                            <span className="text-[10px] text-violet-500 block">μ̃_y (markaziy)</span>
                            <span className="text-2xl font-black font-mono text-blue-600">{{ fuzzyResult.mu_y.toFixed(4) }}</span>
                        </div>
                        <div>
                            <span className="text-[10px] text-violet-500 block">β̃_y (o'ng)</span>
                            <span className="text-xl font-black font-mono text-cyan-600">{{ fuzzyResult.beta_y.toFixed(4) }}</span>
                        </div>
                    </div>
                </div>
            </div>
